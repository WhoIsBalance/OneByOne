...
if __name__ == "__main__":
    ...
    # # 编码序号，drug(0-1079)->protein(1080-5186)->adr(5187-10765)
    # # indication[:,1] += 2926
    # # x = np.zeros((indication.shape[0], 1))
    # # indication = np.concatenate([indication, x], axis=1)    # has_indication:0
    kg[:,1] += 1080     # kg 其他关系(1-49)
    # # kg = np.concatenate([kg, indication]).astype(int)
    mat = np.zeros((1080, 5579))
    for i in range(len(kg)):
        mat[kg[i][0], kg[1][1]] = 1
    x = np.where(mat == 0)
    neg = np.stack(x).T
    y = np.zeros((neg.shape[0], 1))
    neg = np.concatenate([neg, y], axis=1).astype(np.int32)
    drug_se_data = np.concatenate([drug_se_data, neg], axis=0)
    drug_se_data[:,1] += 5186
    ...
    for train_idx, test_idx in kf.split(drug_se_data):
        ...
        x = np.where(train_set[:,2] == 1)
        pos = train_set[x]
        # pos = np.stack(x).T
        # y = np.ones((pos.shape[0], 1))
        # pos = np.concatenate([pos, y], axis=1).astype(np.int32)
        train_kg = np.concatenate([kg, pos], axis=0).astype(int)
        np.random.shuffle(train_kg)
        train_kg[:,2] = 1
